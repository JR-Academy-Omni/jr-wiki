...
- **API 模型 ID** —— Fable 5：`claude-fable-5`；Opus 4.8：`claude-opus-4-8`
- **上下文窗口** —— 两者都是 1M tokens
- **单次最大输出** —— 两者都是 128k tokens
- **输入价格** —— Fable 5：**$10 / MTok**；Opus 4.8：$5 / MTok
- **输出价格** —— Fable 5：**$50 / MTok**；Opus 4.8：$25 / MTok
- **Extended thinking** —— 两者都不支持
- **Adaptive thinking** —— 两者都支持（Fable 5 永远开启）
...
每行格式：**维度** —— Mythos5/Fable5 ／ Opus 4.8 ／ GPT-5.5 ／ Gemini 3.1 Pro（带 `*` 的见下方说明）

- **Agentic 编码 · SWE-Bench Pro** —— **80.3%** ／ Opus 69.2% ／ GPT-5.5 58.6% ／ Gemini 54.2%
- **高难编码 · FrontierCode (Diamond, xhigh)** —— **29.3%** ／ Opus 13.4% ／ GPT-5.5 5.7% ／ Gemini 无数据
- **知识工作 · GDPval-AA** —— **1932** ／ Opus 1890 ／ GPT-5.5 1769 ／ Gemini 1314
- **知识工作（视觉）· GDP.pdf（no tools）** —— **29.8%** ／ Opus 22.5% ／ GPT-5.5 24.9% ／ Gemini 16.7%
- **空间推理 · Blueprint-Bench 2** —— **38.6%** ／ Opus 14.5% ／ GPT-5.5 36.2% ／ Gemini 26.5%
- **工具调用 · AutomationBench** —— **17.4%** ／ Opus 15.5% ／ GPT-5.5 12.9% ／ Gemini 9.6%
- **电脑操作 · OSWorld-Verified** —— 85.0% ／ Opus 83.4% ／ GPT-5.5 78.7% ／ Gemini 76.2%
- **法律 · Legal Agent Benchmark** —— **13.3%** ／ Opus 10.4% ／ GPT-5.5 2.1% ／ Gemini 0.0%
- **跨学科推理 · Humanity's Last Exam（no tools）** —— 59.0%(带星) ／ Opus 49.8% ／ GPT-5.5 41.4% ／ Gemini 44.4%
- **跨学科推理 · Humanity's Last Exam（with tools）** —— 64.5%(带星) ／ Opus 57.9% ／ GPT-5.5 52.2% ／ Gemini 51.4%
- **生物 · BioMysteryBench（hard）** —— 46.1%(带星) ／ Opus 40.0%
- **生物 · BioMysteryBench（human solved）** —— 83.9%(带星) ／ Opus 80.4%
- **Agentic 编码 · Terminal-Bench 2.1** —— 88.0%(带星) ／ Opus 82.7% ／ GPT-5.5 83.4%（Codex CLI）／ Gemini 70.7%（Gemini CLI）
- **网络安全 · ExploitBench（Cap%）** —— 78.0%(带星) ／ Opus 40.0% ／ GPT-5.5 34.0%
- **健康 · HealthBench Professional** —— 66.0%(带星) ／ Opus 56.9% ／ GPT-5.5 51.8%

**怎么读这张表：** 上面标"(带星)"的指标，Mythos 5 和 Fable 5 差距较大——因为对网络安全 / 生物类问题有 blocking 安全护栏，这些项 **Fable 5 经 fallback 后实际更接近 Opus 4.8**，不要当成 Fable 5 的成绩。没标的才是 Fable 5 实打实的水平。详见官方 system card。
...
**注：** 下面这些 Messages API 行为是 Fable 5（和 Mythos 5）**专属**的，Opus / Sonnet / Haiku 不受影响。
...
官方原话：「Adaptive thinking is the only thinking mode on Claude Fable 5 and Claude Mythos 5. It applies whenever the `thinking` parameter is unset, and `thinking: {"type": "disabled"}` is not supported.」
