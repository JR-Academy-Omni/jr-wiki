 ...
 * AI 每日头条海报 · 共享渲染库 · v3
 *
 * v2 → v3 变化：新增 LABELS 配置（kicker / cornerTag / summaryTagline /
 * headlinePrefix / headlineSuffix / summaryFooterLeft / summaryFooterRight /
 * brandSuffix / qrHint），默认值与 v2 完全一致，老数据零改动可直接换 v3。
 * 用途：让非「AI 日报」的内容（如比赛雷达周报）复用同一套画法换文案。
 *
 * v1 → v2 变化：
...
(function () {
  ...
  const LBL = {
    kicker: 'JR ACADEMY · AI 日报',
    cornerTag: '匠人 AI 日历',
    summaryTagline: 'what changed today, why it matters, and what to watch next.',
    headlinePrefix: '头条：',
    headlineSuffix: ' 这条线最值得先看。',
    summaryFooterLeft: 'JR ACADEMY · AI DAILY',
    summaryFooterRight: '#AI-CALENDAR · TOP 5',
    brandSuffix: ' · AI 日报',
    qrHint: '扫码看完整报道 →',
    highlightTag: '今日 AI 日历 · HIGHLIGHT',
    highlightSub: '5 条新闻，按影响力排序。',
    boxBrand: 'JR Academy / AI Daily',
    boxLine1: 'NEWS LIST · TOP 5',
    boxLine2: 'MODEL · PRODUCT · MARKET',
    heroLine2: 'AI 新闻榜，',
    heroLine3Hl: '5 条必看',
  };
  ...
  function drawMarsHeader(ctx, DATE, label) {
    ...
    ctx.fillText(`${LBL.kicker} · ${label || 'TODAY'}`, 88, 77);
    ctx.font = `900 24px ${FF_CN}`;
    ctx.fillStyle = '#10131f';
    ctx.fillText(LBL.cornerTag, 606, 77);
  ...
  function drawSinglePoster(ctx, d, DATE, articleUrl, H, shrinkLevel) {
    ...
    for (let i = 0; i < d.bullets.length; i++) {
      ...
      ctx.fillStyle = '#10131f';
      ctx.fillText(b.k || sectionLabels[i], CX + 28, y + 66);
  ...
  function drawSummaryPoster(ctx, s, DATE, articleUrl, H) {
    ...
    ctx.fillStyle = '#10131f';
    ctx.fillText(LBL.highlightTag, CX + 28, y + 44);
    ctx.font = `900 40px ${FF_CN}`;
    ctx.fillText(LBL.highlightSub, CX + 28, y + 92);

    drawSharpBox(ctx, W - 470, y, 398, 118, '#f7f3ea', '#10131f', 0);
    ctx.font = `900 24px ${FF_CN}`;
    ctx.fillStyle = '#10131f';
    ctx.fillText(LBL.boxBrand, W - 442, y + 40);
    ctx.font = `700 22px ${FF_MONO}`;
    ctx.fillStyle = '#e5261f';
    ctx.fillText(LBL.boxLine1, W - 442, y + 76);
    ctx.fillStyle = '#565967';
    ctx.fillText(LBL.boxLine2, W - 442, y + 104);
    ...
    ctx.fillText(LBL.summaryTagline, CX, y + 34);
    y += 92;

    ctx.font = `900 34px ${FF_CN}`;
    ctx.fillStyle = '#10131f';
    ctx.fillText(`${LBL.headlinePrefix}${firstItem.cat || 'AI'}${LBL.headlineSuffix}`, CX, y);
    y += 50;
    ctx.fillStyle = '#ffd225';
    ...
    ctx.fillStyle = '#565967';
    ctx.fillText(LBL.summaryFooterLeft, CX, H - 34);
    const issue = LBL.summaryFooterRight;
  ...
  function drawFooter(ctx, cx, y, cw, srcText, articleUrl) {
    ...
    ctx.fillText(LBL.brandSuffix, cx + brandW, y + 48 + 52);

    ctx.font = `700 22px ${FF_MONO}`;
    ctx.fillStyle = '#64748b';
    ctx.fillText(LBL.qrHint, cx, y + 48 + 52 + 44);
...
`;
  ...
  async function renderAll(config) {
    ...
    Object.assign(LBL, config.LABELS || {});
